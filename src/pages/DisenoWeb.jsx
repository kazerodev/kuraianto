...
const FEATURES = [
  ...
  { title: "Pago seguro con Stripe", desc: "Proceso de contratación transparente. Pagas de forma segura y empezamos a trabajar enseguida, sin letra pequeña." },
...
const FAQS = [
  { q: "Cómo se realiza el pago?", a: "El pago se realiza de forma segura a través de Stripe. Una vez confirmado, acordamos el calendario y empezamos enseguida. Antes de empezar, te enviamos el contrato completo." },
          ...
          <Reveal>
            <div className="max-w-3xl">
              ...
              <p className="text-neutral-400 text-lg leading-relaxed mb-8 max-w-xl">
                No diseñamos webs para que queden bonitas. Las diseñamos para que vendan. Pago seguro con Stripe, sin permanencias, desde 349€.
              </p>
              ...
              <Reveal key={plan.name} delay={i * 80}>
                <div className={`rounded-2xl p-7 h-full flex flex-col ${plan.popular ? "border border-orange-500/30 bg-orange-500/[0.05]" : "border border-white/[0.07] bg-white/[0.025]"}`}>
                  ...
                  <a
                    ...
                    className={`block text-center font-bold py-3.5 rounded-full text-sm transition-all ${plan.popular ? "bg-orange-500 hover:bg-orange-400 text-white" : "bg-orange-500/10 hover:bg-orange-500/20 text-orange-400 border border-orange-500/20"}`}
                  >
                    Contratar ahora
                  </a>
                  <a
                    href={wa(`Hola, me interesa el plan ${plan.name} (${plan.price}). ¿Podéis contarme más detalles?`)}
                    target="_blank"
                    rel="noopener noreferrer"
                    data-event={`diseno-wa-${plan.name.toLowerCase().replace(/\s+/g, "-")}`}
                    className="block text-center font-semibold py-3 rounded-full text-sm text-neutral-400 hover:text-white transition-colors mt-2 text-xs"
                  >
