...
const FEATURES = [
  ...
  { title: "Contratación por WhatsApp", desc: "Escríbenos, te enviamos la factura directamente y empezamos enseguida. Sin formularios complicados, sin letra pequeña." },
...
const FAQS = [
  { q: "Cómo se realiza el pago?", a: "Escríbenos por WhatsApp indicando el plan que quieres y te enviamos la factura directamente. Una vez pagada, acordamos el calendario y empezamos enseguida. Antes de empezar, te enviamos el contrato completo." },
          ...
          <Reveal>
            <div className="max-w-3xl">
              ...
              <p className="text-neutral-400 text-lg leading-relaxed mb-8 max-w-xl">
                No diseñamos webs para que queden bonitas. Las diseñamos para que vendan. Contratación por WhatsApp, factura al momento, desde 349€.
              </p>
              ...
              <div className="flex flex-wrap items-center gap-5 mt-8">
                {["Factura por WhatsApp", "Desde 349€", "Lista en 7-14 días"].map((item) => (
            ...
            {PLANS.map((plan, i) => (
              <Reveal key={plan.name} delay={i * 80}>
                <div className={`rounded-2xl p-7 h-full flex flex-col ${plan.popular ? "border border-orange-500/30 bg-orange-500/[0.05]" : "border border-white/[0.07] bg-white/[0.025]"}`}>
                  ...
                  <a
                    href={wa(`Hola, quiero contratar el plan ${plan.name} (${plan.price}). ¿Podéis enviarme la factura para empezar?`)}
                    target="_blank"
                    rel="noopener noreferrer"
                    data-event={`diseno-wa-${plan.name.toLowerCase().replace(/\s+/g, "-")}`}
                    className={`flex items-center justify-center gap-2 w-full font-bold py-3.5 rounded-full text-sm transition-all hover:-translate-y-0.5 ${plan.popular ? "bg-orange-500 hover:bg-orange-400 text-white" : "bg-orange-500/10 hover:bg-orange-500/20 text-orange-400 border border-orange-500/20"}`}
                  >
                    ...
                    Contratar por WhatsApp — {plan.price}
                  </a>
                </div>
              </Reveal>
            ))}
